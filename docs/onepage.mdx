...
4. **Deploy to Testnet:** Run `zk config`, which will walk you through adding a
   ...
   `https://proxy.berkeley.minaexplorer.com/graphql` for the url. Then run `zk deploy` and follow the prompts. See the [how to deploy a zkApp](/) page for further details.
...
One such workaround is the use of actions – see [Actions and Reducer](/).
...
The list above just reflects common defaults. The full source of truth is given by the _account permissions_, see [Permissions](/). Using permissions, account owners can decide on a fine-grained level which type of authorization is required on which kinds of updates - take a look at the [Advanced SnarkyJS Permission](/) section that goes into detail!.
...
Your website needs to contain the `smart_contract.js` file that was generated when building your smart contract. On the [how to write a zkApp](/) page, we will go into more detail.
...
Join the [#zkapps-developers](https://discord.com/channels/484437221055922177/915745847692636181) channel on Mina Protocol Discord. Participate and ask questions in [zkApps Developers Office Hours](/).
...
To do so, just create an [account update](/) — you'll find the same `account` / `network` fields API on it:
...
3. The [Mastermind game](/) uses linear recursive proofs, an example of an application-specific rollup, to progress the state-machine of the application without needing to sync back to the game.
...
This just adds the basic setup for our smart contract. For more details on the `deploy()` method — see [Tutorial 1: Hello World](/).
...
[How to write a zkApp](/) provides a high-level overview of everything you need to know to build applications
...
<!--
One of the benefits of writing smart contracts in TypeScript, is that the compiled JavaScript and WASM smart contract is easy to [integrate into your website UI](/).
