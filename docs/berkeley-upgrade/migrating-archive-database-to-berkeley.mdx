...
7. Stage 3: Remainder migration
   ...
   :warning: 3NLdCBNrDseiDKvVj8rZ15k2oAUvx4XuCc8mzf6fL2CmqTJVVceM is only an example random hash. **Do not use this example hash** on the actual migration. **Use the official hash** as provided by Mina Foundation for the fork point.
...
:info: We are tightly coupled with Google Cloud infrastructure due to the precomputed block upload mechanism.
