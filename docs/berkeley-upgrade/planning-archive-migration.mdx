...
hide_title: true
description: Mina protocol heavily relies on its archive node. It is crucial to properly migrate archives from Mainnet to Berkeley.
keywords:
  - Berkeley
  - upgrade
  - archive migration
...
Archive node migration is a crucial part of Berkeley upgrade. We need to convert mainnet database to berkeley one in order to preserves historical data and assure archive node chain continuity.
For this purpose o1Labs and mina-foundation teams prepared two packages:

### Archive Node Berkeley migration package

This package contains applications needed to migrated existing mainnet database into new bekeley schema.
...
The berkeley-migration and replayer applications are able to work incrementally so that part of the Mainnet database can be migrated and, as new blocks are added to the Mainnet, the new data in the database can be migrated.
...
### Mainnet Node Archive maintenance package

Archive Node Berkeley migration package is sufficient for staisfying migration from mainnet to Berkeley.
...
For this purpose we are also releasing mainnet archive maintenance package for those Archive Node operators who wants to still mainntain copy of mainnet database for historical reasons.
...
#### (Optional) Mainnet database 

One of the most obvious prerequisite is a mainnt database.
If you don't have an existing database with Mainnet archive data, you can always download it from our Google Cloud bucket. However, we strongly encourage to perform migration on your own data in order to preserve idea of decentralization
As mentioned

#### (Optional) Google Cloud bucket with Mainnet precomputed blocks
...
1. Download the Mainnet archive data using cURL or gsutil:
...
### Ensure location of Google Cloud bucket with Mainnet precomputed blocks
