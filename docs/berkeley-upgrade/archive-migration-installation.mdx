...
To mitigate these limitations, the archive maintenance package is available for use by archive node operators who want to maintain a copy of their Devnet and Mainnet databases for historical reasons.
...
We strongly encourage you to perform the migration on your own data to preserve the idea of decentralization. However, if you want to use archive data o1labs runs (for example, bootstrapping a new archive from SQL without waiting all day for the chain to download and replay), use the following steps.
...
1. Download the Devnet/Mainnet archive data using cURL or gsutil:
    ...
    - cURL:
     ...
     To filter the dumps by date, replace `{date}` using the required `yyyy-dd-mm` format. For example, for January 15, 2024, use `2024-01-15`.
  
     :warning: The majority of backups have the `0000` suffix. In case a download with that name is not available, try incrementing it once or twice. For example, `0001`, `0002`.
...
The recommended method is to perform migration on your own data to preserve the idea of decentralization.
...
For more information, see [How to maintain archive data](/berkeley-upgrade/mainnet-database-maintenance).
...
Choose the appropriate packages for your environment. 
...
CODENAME=bullseye
CHANNEL=devnet
VERSION=3.0.0devnet-tooling-5792b50
...
Congratulations on completing the essential preparation and verification steps. You are now ready to perform the migration steps in [How to perform archive migration](/berkeley-upgrade/migrating-archive-database-to-berkeley).
