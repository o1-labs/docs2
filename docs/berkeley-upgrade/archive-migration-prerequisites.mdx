...
If you don't have an existing database with Devnet/Mainnet archive data, you can always download it from the Google Cloud bucket. However, we strongly encourage you to perform migration on your own data to preserve the idea of decentralization.
...
Precomputed blocks are the JSON files that a correctly configured node will updload to the Google Cloud bucket.
...
However, it is strongly recommended that you perform migration on your own data to preserve the idea of decentralization:
...
:warning: Precomputed blocks for the 'devnet' network take several hundred GBs. Plan for adequate time to download these blocks. You can instead download a ~50 GB bundle of only the canonical `devnet` blocks that unpacks into ~90 GB: 
...
These bundles are partial. Look for updated documentation with the new links and final data _after_ the Berkeley major upgrade is completed.

The best practice is to collect precomputed blocks by yourself or by other third parties to preserve the idea of decentralization. 
