...
description: Use SnarkyJS to retrieve previously emitted events and actions from Mina Archive Node. Connect your zkApp smart contract to an Archive Node API endpoint and fetch and use events and actions.
keywords:
  - mina archive node
  - snarkyjs
  - fetch events
...
To see an end-to-end example of zkApp fetching events from a running network, see the example [run_berkeley.ts](https://github.com/o1-labs/snarkyjs/blob/main/src/examples/zkapps/voting/run_berkeley.ts) file that is provided in the codebase repo.
