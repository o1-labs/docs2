...
The best way experience Lightnet is by using it from the zkApp CLI command line.
...
- Starts the Mina archive process and the [archive-node-API](https://github.com/o1-labs/Archive-Node-API) application.
   This application runs alongside other applications in the Docker contain and talks to the PostgreSQL database, which is populated with data by the mina archive process.
...
This command downloads and updates a lightweight Mina explorer. By default, the latest version is downloaded (if needed) and launched. 
