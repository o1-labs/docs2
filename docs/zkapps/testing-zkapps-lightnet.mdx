...
### Start a local network without the archive node

The default when starting Lightnet is to start an archive node with a PostgreSQL service, the Mina archive process, and the Archive-Node-API application.

To use fewer resources when your testing does not require an archive node, you can start a local network without the archive node. 

To disable the archive node:
...
To monitor transactions and visualize the , launch the lightweight Mina Explorer:
...
You can add contributions, create feature requests, and let us know about problems by creating issues for Lightnet in the https://github.com/o1-labs/zkapp-cli repository. Be sure to add the `lightnet` label. 