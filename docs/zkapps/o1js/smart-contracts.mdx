...
key stored on the account that can verify zero knowledge proofs generated
...
The API is like `console.log`, but it automatically handles printing o1js data types in a readable format. However, it does nothing during `SmartContract` compilation.
...
One workaround is to use actions. See [Actions and Reducer](/zkapps/o1js/actions-and-reducer).
...
If it succeeds, it becomes part of the proof that is verified on-chain.
...
You can create a custom data type for your smart contract using the `Struct` function that o1js exposes. Create a class that extends `Struct({ })`.
...
Create a Point class by creating a new class that extends the `Struct` class:
...
Note that your Structs can contain o1js built-in types like `Field`, `Bool`, `UInt64`, and so on, or even other custom types that you've defined which are based on the `Struct` class. 
