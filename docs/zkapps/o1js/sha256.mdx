...
In contrast, SHA2 is a hash function that requires binary arithmetic. It operates over binary data and is not native to most zero knowledge proofs. For this reason, Keccak is not as efficient as Poseidon, but it is still very useful for verifying Ethereum transactions and blocks.
...
SHA256 is available in the following configuration that are available under the `Hash` namespace in o1js:

- `Hash.SHA2_256`: SHA2-256 hash function with output size of 256 bits.

Because SHA2 operates over binary data instead of native Field elements like Poseidon, o1js uses the `Bytes` type. `Bytes` is a fixed-length array of bytes that can be used to represent binary data.
