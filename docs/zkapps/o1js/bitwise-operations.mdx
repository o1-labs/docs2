...
- Is implemented in two different ways:
    ...
    - If the `checked` parameter is set to `true`, the [xor()](#xor) gadget is reused with a second argument to be an all-ones bitmask of the same length. This approach is less efficient, needing as many rows as an XOR requires for a single negation, but comes with the advantage of making sure the input is of a certain length.
...
The `addMod32()` operation adds two Field elements in the range `[0, 2^64]` and returns the result modulo `2^32`.
...
The `divMod32()` gadget adds two Field elements in the range `[0, 2^64]` and returns the result modulo `2^32`.

The operation:

- Is division modulo `2^32` (provides the remainder of the division when a number is divided by `2^32`).
