...
- Is implemented in two different ways:
    - If the `checked` parameter is set to `false`, it is implemented as a subtraction of the input from the all-ones bitmask (all the bits in a binary sequence are set to 1). If the `checked` parameter is not provided, the default implementation uses `false`.
