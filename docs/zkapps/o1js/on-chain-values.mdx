...
In a zkApp, you can access the current [on-chain state](/zkapps/how-to-write-a-zkapp#on-chain-state) of a zkApp account and many other on-chain values.

The zkApp account gets updated on-chain. On the Mina blockchain, each zkApp account provides eight fields of 32 bytes each of arbitrary storage for the on-chain state.
