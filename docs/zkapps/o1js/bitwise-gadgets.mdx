...
- Specifying a larger `length` parameter adds additional constraints.
...
The bitwise compactMultiRangeCheck gadget is a variant of [multiRangeCheck](#multirangecheck) where the first two variables are passed in combined form `xy = x + 2^88*y`. It is similar to the [Left shift (<<)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Left_shift) operation in JavaScript. ,
...
The multiRangeCheck gadget building block for non-native arithmetic with BigInt of size up to 264 bits. The multiRangeCheck gadget is a provable method for efficient 64-bit range checks using lookup tables.
...
The rotate gadget operates similarly to the [Right shift (>>)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Right_shift) and [Left shift (<<)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Left_shift) operators in JavaScript. 
