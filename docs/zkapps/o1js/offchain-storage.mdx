...
The StateProof type will subsequently be employed to finalize published state changes using a recursive reducer mechanism.
...
Furthermore, the smart contract requires a field indicating the commitment to the offchain state. This field facilitates accessing offchain state similar to any other on-chain value.
...
The contract also necessitates a settle() method to resolve all pending state updates. This method employs a recursive proof to finalize all pending state changes, with the proof generated before invoking the settle() method.
...
### Preparing for Offchain State Settlement

Additionally, developers need to set the smart contract instance and assign it to the offchain storage as follows:
...
Additionally, the Offchain storage automatically compiles the recursive Offchain zkProgram in the background and assigns the Offchain state to a smart contract instance.
