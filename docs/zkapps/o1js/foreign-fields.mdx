...
The only parameter that `createForeignField()` takes is the modulus or size of the field. This code example passes in `17n` so that `SmallField` allows you to perform arithmetic modulo 17:
