...
o1js produces `kimchi` proofs.  Kimchi is defined in detail in the [Mina Book](https://o1-labs.github.io/proof-systems/specs/kimchi.html), including specifications for each type of gate that is supported.  Generally speaking, each type of gate represents a specific mathetmatic expression, and the values in the row represent coeffecients of that expression.
...
There is no equivalent to `JUMP` or `GOTO` in a circuit.  The implication of this for the developer is there is no way to branch or loop in o1js.  There are some workarounds to these limitations that we will discuss, but it's important to understand the fundamental limitation.
...
You see, `Provable.constraintSystem` let's us visualize our entire circuit.  When using `Provable.constrainSystem`, witnesses need to be explicitly created for any input data.  This is easily verified by updating the code:
...
As you can see, there is no circuit this time.  This showcases another benefit of checking your code with `Provable.constraintSystem` - it can help you verify that your circuit is actually provable.
