...
class Leaderboard extends SmartContract {
  // the root it the root hash of our off-chain Merkle tree
...
let circuitWitness = new MerkleWitness(witness);

// calculates the root of the witness
let calculatedRoot = circuitWitness.calculateRoot(Field.fromNumber(123));
