...
`access`: The permission corresponding to the ability to access a ledger account.
...
in order to prevent a malicious actor from changing the permission `setVerificationKey` to, for example, `none` - which would allow an actor to manipulate the verification key again.
