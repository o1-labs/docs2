...
Because `none` means we don't have to provide *any* from of authorization, a malicious actor can easily drain all funds from the smart contract! Take a look at the following transaction that abuses that:
...
We remember that one permission was called `setVerificationKey` - by modifying the authorization kind for this permissions, we can make our smart contract upgradeable or not. 
