...
By setting the `archive` property to one of the listed endpoints, SnarkyJS uses one of the selected endpoints to fetch events. For example, if a zkApp has already emitted events and has broadcasted the transaction. To fetch events for a zkApp, you can use the `fetchEvents` function:
...
`getEvents` takes in a starting block and an ending block. 
