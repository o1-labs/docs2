...
Since Mina nodes do not store historical network information and events are not kept on-chain, emitted events are preserved only in the archive node. To see how to work with events within your zkApp, refer to our [Using the Archive Node with SnarkyJS guide](../../node-operators/archive-node#using-the-archive-node-with-snarkyjs).
...
To see a end-to-end example of zkApp fetching events from a running network, [see this voting example code that fetches preivously emitted events](https://github.com/o1-labs/snarkyjs/blob/main/src/examples/zkapps/voting/run_berkeley.ts).
