...
Since Mina nodes do not store historical network information and actions are not kept on-chain, emitted actions are preserved only in the archive node. To see how to work with actions within your zkApp, refer to our [Using the Archive Node with SnarkyJS guide](../../node-operators/archive-node#using-the-archive-node-with-snarkyjs).
