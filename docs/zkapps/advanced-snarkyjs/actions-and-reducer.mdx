...
`getActions` takes an optional `fromActionHash` parameter that is the `actionsHash` of the last action that was processed. If `fromActionHash` is not provided, `getActions` fetches all actions for the zkApp. `getActions` returns a list of actions that were emitted after the `fromActionHash` was processed.
