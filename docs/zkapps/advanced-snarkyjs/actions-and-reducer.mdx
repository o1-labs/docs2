...
Since Mina nodes do not store historical network information and actions are not kept on-chain, emitted actions are preserved only in the archive node. Consequently, a zkApp must fetch its previously emitted actions on the Mina network from an archive node.
