...
By setting the `archive` property to one of the listed endpoints, SnarkyJS uses one of the selected endpoints to fetch action data. For example, you can assume that a zkApp has already emitted actions and has broadcasted the transaction. To fetch actions for a zkApp, you can use the `getActions` function:
