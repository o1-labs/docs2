...
Since Mina nodes do not store historical network information and actions are not kept on-chain, emitted actions are preserved only in the archive node. To see how to work with actions within your zkApp, refer to our [Using the Archive Node with SnarkyJS](../../node-operators/archive-node#using-the-archive-node-with-snarkyjs) section.
...
To see a end-to-end example of zkApp fetching actions from a running network and including them into your zkApp, see this voting app example in the [voting.ts](https://github.com/o1-labs/snarkyjs/blob/main/src/examples/zkapps/voting/voting.ts) file in examples provided in the codebase repo.
