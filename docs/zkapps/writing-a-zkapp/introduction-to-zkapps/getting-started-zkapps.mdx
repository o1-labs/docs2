...
You can start writing zkApps with just a few steps. You can write zkApps for your project in other ways. The focus of this Getting Started Guide is a high-level workflow to build and deploy quickly.
...
1. [Create integration tests and test locally](#5-create-an-integration-test)
1. [Test with Lightnet](#6-test-with-lightnet)
...
### 6. Test with Lightnet

To test your zkApp with an accurate representation of Mina blockchainwith, start Lightnet:

1. Spin up a Lightnet blockchain:

    ```sh
    zk lightnet start
    ```

    In most cases, a single node is appropriate for testing. Use the start configuration options appropriate for your use case. See [Start a local network with other settings](/zkapps/lightnet#start-a-local-network-with-other-settings).
...
1. Communicate with the Account Manager service to fetch account details.

1. Configure your zkApp for Lightnet blockchain. Use the helpful URLs provided by the `zk lightnet status` command.     
