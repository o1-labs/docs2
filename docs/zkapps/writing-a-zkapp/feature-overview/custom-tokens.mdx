...
For a more comprehensive example of how to use custom tokens with a zkApp, see this custom token example provided in [token.test.ts](https://github.com/o1-labs/o1js/blob/main/src/lib/token.test.ts).

## Proof Authorization

When a zkApp interacts with a custom token that it did not originally create, the calling zkApp must get authorization from the [token owner](/zkapps/o1js/custom-tokens#token-owner).
