...
This function to retrieves a list of previoulsy emmited actions from an archive node:

```ts
this.reducer.getActions({ fromActionState?: Field, endActionState?: Field }): MerkleList<MerkleList<Action>>;
```
