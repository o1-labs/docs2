...
zkApp programmability is not available on the Mina Mainnet. You can get started now by deploying zkApps to the Berkeley Testnet.
...
It's good practice to break down the functionality of your smart contract into `describe` blocks to organize your test groupings in the same file. Mapping out your unit tests using the `describe` convention provides documentation to developers who read your smart contract. 
...
For basic test examples that deploy and update the state on a smart contract using Jest, see [Add.test.ts](https://github.com/o1-labs/zkapp-cli/blob/main/templates/project-ts/src/Add.test.ts).
...
The `Mina.LocalBlockchain()` method takes an optional parameter `{ proofsEnabled: true }` to specify whether to generate and verify proofs. 
