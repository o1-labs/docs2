...
4. **Deploy to Testnet:** Run `zk config`, which will walk you through adding a
   ...
   (how-to-deploy-a-zkapp) page for further details.
...
One more note about private inputs: The method above has one input parameter, `x` of type `Field`. In general, arguments can be any of the built-in SnarkyJS type that you saw: `Bool`, `UInt64`, `PrivateKey`, etc. From now on, we will refer to those types as [circuit values](how-to-write-a-zkapp#custom-data-types).
...
One such workaround is the use of actions -- see [Actions and Reducer](advanced-snarkyjs/actions-reducer).
