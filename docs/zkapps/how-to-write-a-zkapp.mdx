...
All of this is true because `this.send()`, under the hood, placed the receiver update at call depth 1, under the zkApp update. As a counter-example: The fee payer is _not_ part of the zkApp sub-tree, and likewise does not become part of the public input. It can be anything without affecting the validity of the proof.

A key takeaway is: If you want something to become part of your proof, you have to put it inside your `@method`. Things that happen outside your methods won't be part of any proof, so you can't rely on those things being part of your smart contract logic.
