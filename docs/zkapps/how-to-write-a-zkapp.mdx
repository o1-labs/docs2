...
If you haven’t yet read [how zkApps work](how-zkapps-work), please read it now so that this section makes sense.
...
To view the full SnarkyJS reference, see the [snarkyJS reference](snarkyjs-reference) docs.
...
Instead, use the SnarkyJS built-in `Provable.if()` method, which is a ternary operator:
...
Some common methods you use often are:
...
Now that you know the basics of writing SnarkyJS programs, here's how to create a smart contract.
...
contract deployed to it that essentially just means there's a verification
...
To get an idea what "successful execution" means, look at this line in the previous example:
...
Magically, the proof can be checked without seeing `x` because `x` is a _private input_.

This method is not very meaningful yet. To make it more interesting, you need a way to interact with accounts and record state on-chain.

One more note about private inputs: The previous method has one input parameter, `x` of type `Field`. In general, arguments can be any of the built-in SnarkyJS type: `Bool`, `UInt64`, `PrivateKey`, and so on. From now on, those built-in SnarkyJS types are referred to as [structs](how-to-write-a-zkapp#custom-data-types).
...
<!-- TODO: create at least a basic section about zk circuits below this subsection which contains (among other things) a more thorough explanation of what this tip hints at -->

:::tip

Inside a `@method`, things sometimes behave a little differently. For example, the following code can't be used in a method where `x: Field` is an input parameter:

```ts
console.log(x.toString()); // don't do this inside a `@method`! 😬
```

This doesn't work because, when we compile the SmartContract into prover and verification keys, your method runs in an environment where the method inputs don't have any concrete values attached to them. They are like mathematical variables `x`, `y`, `z` that are used to build up abstract computations, like `x^2 + y^2`, just by running your method code.

Therefore, when executing your code and trying to read the value of `x` to turn it into a string via `x.toString()`, the execution fails because the value can't be found. In contrast, during proof generation all the variables _have_ actual values attached to them (cryptographers call them "witnesses"). It makes perfect sense to want to log these values for debugging.
This is why a special function is available to log stuff from inside your method:

```ts
Provable.log(x);
```

The API is like that of `console.log`, but it automatically handles printing SnarkyJS data types in a nice format. During SmartContract compilation, it simply does nothing.
:::

##### On-chain state

A smart contract can contain **on-chain state** that is declared as a property
...
Often, you also want to _read_ state as shown in this example:
...
Later, it sets the new state to `x + 1` using `this.x.set()`. 

There's another line that looks weird at first:

```ts
this.x.assertEquals(x);
```

To understand it, take a step back and understand what it means to "use an on-chain value" during off-chain execution.

To use an on-chain value, you have to _prove_ that this is the on-chain value. Verification has to fail if it's a different value! Otherwise, a malicious user could modify SnarkyJS and make it just use any other value than the current on-chain state – breaking the zkApp.
...
- If the assertion fails, SnarkyJS throws an error and does not submit the transaction.
- If the assertion succeeds, it becomes part of the proof that is verified on-chain.

Because of this, the new version of `increment()` is _guaranteed_ to behave like the previous version: It can only ever
update the state `x` to `x + 1`.

:::tip
You can add optional failure messages to assertions to make debugging easier. For example, the previous example could be written as:
...
Assertions can be incredibly useful to constrain state updates. Common assertions you can use are:
...
The only way method parameters can be exposed is when the computation explicitly exposes them, like 
