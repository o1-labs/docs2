...
Let's continue our discussion of account updates from the preceding sections, at the hand of an example that is important on its own: Paying out MINA from a zkApp. To send MINA, there is `this.send()`, which you can use inside your smart contract method:
