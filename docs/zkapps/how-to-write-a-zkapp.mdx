...
Let's recap: We have explained how to write a SmartContract. Then we've seen how to create a transaction which calls that contract, and how the transaction consists of account updates which where created by SnarkyJS under the hood. Now, we'll see an example where we have to create an account update explicitly. We'll also learn how to use signatures, for authorizing updates to user accounts.

We'll continue the payment topic. Last section, we paid out MINA from the zkApp. This time, we do the other direction: make a deposit from the user into the zkApp. Here's the smart contract code:
...
Note that `AccountUpdate` is the class in SnarkyJS that represents account udpates. `AccountUpdate.create()` not only instantiates this class, but also attaches it to the current transaction, at the same level where `create` is called. If it is called inside a `@method`, the `AccountUpdate` is created as a child (public input) of the zkApp update.

The next line tells SnarkyJS that this update will be authorized with a signature later:

```ts
senderUpdate.requireSignature();
```

We'll get into this later. Instead of writing `AccountUpdate.create()` and `requireSignature()`, we could've used a shortcut which does both of them in one command:
...
This is similar to the output in the last section, with the balance changes reversed. The only other differences are on the user update (third in the list):

- `useFullCommitment: true`, which we won't explain here but has to do with replay protection if you're using signatures.
- `authorizationKind: 'Signature'`

Those two changes were caused by calling `requireSignature()` on the account update. `authorization: undefined` indicates that we didn't provide this signature yet.

In a real zkApp, user signatures will be added by the wallet, not within SnarkyJS. In that case, the missing signature is fine. However, for testing and calling zkApps via nodeJS, you need to add those signatures yourself. The command to do that is `tx.sign([...privateKeys])`, called after `Mina.transaction` on the finished transaction. Here's a full example:
...
tx.sign([senderKey]); // senderKey is a PrivateKey
```
