...
```ts
const senderKey = PrivateKey.fromBase58('EKEQc95...');
```
...
The list above just reflects common defaults. The full source of truth is given by the _account permissions_, see [Permissions](snarkyjs-reference/Interfaces/Permissions). Using permissions, account owners can decide on a fine-grained level which type of authorization is required on which kinds of updates.

<!-- TODO: link to advanced permissions section instead of just API reference; this section should list all permissions, explain how permissions are changed through zkApp deployment, explain how accounts can be made non-upgradeable, and explain the `access` permission for the case of token contracts -->
...
// our accounts updates are applied on chain!
```
...
Everything else works the same locally as it would when interaction with a real network. You still need to properly sign your transactions and attach proofs, and then send them with `tx.send()` to apply them locally. To help you iterate quicker when testing, there's an option to disable proof creation: `Mina.LocalBlockchain({ proofsEnabled: false })`. You can use it like this:

```ts
// set Mina instance to local
const proofsEnabled = false;
const Local = Mina.LocalBlockchain({ proofsEnabled });
Mina.setActiveInstance(Local);

// only compile smart contract when proofs are enabled
...
Note that with `proofsEnabled: false`, you still need to call `await tx.prove()` like normally. The only change is that `.prove()` creates a dummy proofinstead of a real one, and so runs in 0 seconds. Also, `tx.send()` will skip the usual step of verifying your proof.

You can learn more in [How to test your zkApp](how-to-test-a-zkapp).
