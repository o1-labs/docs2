...
1. **Install:** Run `zk project <myproj>`. Type `ls` and hit enter to see the
  ...
  _Note_: If you want to directly scaffold an UI with your new zkApp, you can do so by adding the `--ui` flag to the previous command, like `zk project <myproj> --ui=<framework>`. You can currently choose from the following frameworks: `svelte`, `next` or `nuxt` 
