...
You can enable these headers a number of different ways. For example, if you deploy your UI to a host such as [Vercel](https://vercel.com/) or [Cloudflare Pages](https://pages.cloudflare.com/), you can set these headers in a custom configuration file. Otherwise, you can set these headers in the server framework of your choice (e.g. Express for JavaScript).
...
the <a
href="https://www.npmjs.com/package/@aurowallet/mina-provider">`auro-web3-provider`</a>
...
try {
  ...
  let { hash } = await window.mina.sendTransaction({
