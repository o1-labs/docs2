...
In the [Hello World](hello-world) tutorial, you built a basic zkApp smart contract with SnarkyJS. You learned how to use the `zk config` command to create a deploy alias in [Tutorial 3: Deploy to a Live Network](deploying-to-a-network).
...
- [Mina zkApp CLI](https://github.com/o1-labs/zkapp-cli) version 0.9.0
- [SnarkyJS](https://www.npmjs.com/package/snarkyjs) 0.11.0
...
1. Install the Auro Wallet for Chrome that supports interactions with zkApps. See [Install a Wallet](using-mina/install-a-wallet).
1. Create an account.

## Use the working application first

Before you go through the tutorial steps, take a look at a working instance that is deployed to GitHub Pages. 
...
1. When prompted, select **Connect** to allow the website to view your Auro wallet account.

--- __QUESTION?? what terminal? what directory?__ ---

1. Extra info is printed to the console as well when it is run, to give an idea of what it is doing internally.
...
2. Checks if the public key has funds, and if no, directs the user to the faucet.
...
Like previous tutorials, you use the provided helper files, so you can focus on the React implementation itself. 

This example uses an RPC endpoint.

## Create a project

The `zk project` command has the ability to scaffold the UI for your project. For this tutorial, use 
...
1. At the `? Would you like to use ESLint with this project? › No / Yes` prompt, select `Yes`.

1. At the `? Would you like to use Tailwind CSS with this project? › No / Yes` prompt, select `Yes`.
...
For this tutorial, you run commands from the root of the `04-zkapp-browser-ui` directory as you work in the `ui/pages` directory on files that contain the UI code. 
...
Because SnarkyJS code is computationally intensive, it's helpful to have the SnarkyJS code in web worker helper files. A web worker handles requests from users to ensure the UI thread isn't blocked during long computations like compiling a smart contract or proving a transaction.

--- __QUESTION?? I didn't have a pages directory?__ ---

1. Create the `pages` directory in `/04-zkapp-browser-ui/ui`.

1. Download the helper files to your local `/04-zkapp-browser-ui/ui/pages` directory:
...
1. Open each helper file to understand how they work and how you can extend them for your own zkApp.
...
1. Copy the `styles` directory from `https://github.com/o1-labs/docs2/tree/main/examples/zkapps/04-zkapp-browser-ui/ui/styles` to your local `/04-zkapp-browser-ui/ui/` directory.

1. This code at the end of `ui/styles/globals.css` file styles the `<a>` link in the application:
...
--- __QUESTION?? I didn't have a pages directory? so just copy one file at a time? or?__ ---

1. Copy the `_app.page.tsx` file from `https://github.com/o1-labs/docs2/tree/main/examples/zkapps/04-zkapp-browser-ui/ui/pages` to your local `pages` directory.
