...
Like in tutorial 3, we will provide a couple helper files, so we can focus on the react implementation itself. What is special about these helpers though, is that they use a web worker. This ensures the UI thread isn't blocked during long computations like compiling a smart contract or proving a transaction.
...
This project requires the latest version of [Auro](https://www.aurowallet.com/) (>= 2.1.2). This version is currently under review at the chrome app store at the time this tutorial was last updated, and will probably be on the chrome store by the time you are reading this.

However if the chrome store hasn't been updated yet, Auro 2.1.2 can be found [here](https://github.com/aurowallet/auro-wallet-browser-extension/releases/), with instructions to install on chrome [here](https://superuser.com/questions/247651/how-does-one-install-an-extension-for-chrome-browser-from-the-local-file-system).
