...
1. Create a project by using the `zk project` command:
  ...
  The Next.js React framework gives you building blocks to create web applications. When you select `next`, the `zk project` command creates the UI for your zkApp. 
...
1. Change to your local project `/04-zkapp-browser-ui/ui` directory:
...
1. Install the dependencies in the `/04-zkapp-browser-ui/` directory:
