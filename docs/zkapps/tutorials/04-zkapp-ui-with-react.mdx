...
Because SnarkyJS code is computationally intensive, put the SnarkyJS code in a web worker with helper files. A web worker handles requests from users to ensure the UI thread isn't blocked during long computations like compiling a smart contract or proving a transaction.
