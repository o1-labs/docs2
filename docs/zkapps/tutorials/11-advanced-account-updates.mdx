...
2. The account update must set `mayUseToken` to True. However, an account update on a token account cannot set this property to true. It must get its account update “approved” by its corresponding manager account. The manager account can approve accounts based on its `access` permission.
...
The special thing to note here is the `approveDeploy(...)` on the `myTokenInstance()` method. Because `tokenHolderInstance()` is going to exist as a token account, it needs approval to be deployed.
...
This checks that the deployUpdate is a single account update, with no children, that it isn't creating any additional tokens, and that it is starting out with a balance of zero.
...
The `fundNewAccount` call is included as the deployerAccount,  in this case it doesn't have an existing token account.
...
This uses a helper `assertHasNoBalanceChange` that walks the accountUpdates being approved and ensures that the balance changes haven't created or destroyed any tokens.

The account updates for this transaction look like:

<img src="/img/tutorial-11-txn2.png" width="95%" />

This shows both the account update for the fee payment and the main chain that does the token operations. You can see the call to `tokenUser` at the top, followed by a call to `myToken` that is doing the approving. This approves two accountUpdates: one update takes tokens away from the `tokenUser` and has the `mayUseToken` field on, and the other update sends tokens to the deployer account.

Because of the assertions in the `myToken` approval account, the balances must match between these two account updates. Which it does, and so a valid `proof` can be constructed.
