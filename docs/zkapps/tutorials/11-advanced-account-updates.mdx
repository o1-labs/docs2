...
Conversely, note that a user can call this method with _any account update they like_, if it only has a balance change of zero. For example, a user could make their 8-field on-chain state whatever they want it to be. In other words, the balance change is the only property of token accounts that our token manager contract chooses to "manage".

:::note

The approval mechanism gives developers complete freedom in encoding constraints on account updates in their manager contract logic. The "fungible token" use case presented here is just one example of what you could do with it.

A different manager contract could, for example, store NFTs (or a commitment to NFTs) in the on-chain state of user accounts. In that case, the on-chain state would be the meaningful part of token accounts. The manager contract woudn't care about constraining the balance change, and instead the approval logic would contain checks that the on-chain state is only updated in a certain way:
...
This shows both the account update for the fee payment - and the main tree that does the token operations. You can see the call to `tokenUser` at the top, followed by a call to `myToken` that is doing the approving. This approves two account updates: one update takes tokens away from the `tokenUser`, and the other update sends tokens to the deployer account. Both have the `mayUseToken` field set to `parentsOwnToken`.
