...
description: Advanced features of account updates: tokens, zkApp manager accounts, and account update assertions.
keywords:
  - smart contracts
  - zkapps
  - account updates
...
Internally, it's more general. The Manager Account for a token actually controls all properties of token accounts.
...
2. The account update must set `mayUseToken` to a truthy value. However, for this to happen, a transaction must get its account update “approved” by the corresponding manager account. The manager account can approve accounts based on its `access` permission.
...
To implement, you need 3 contracts:
...
3. `TokenHolder`: A zkApp contract that stores tokens on behalf of the `TokenUser` contract. This contract is instantiated on the `MyToken` instance tokenId.
...
As shown in the example file, a token standard library abstracts many of these details away for standard use. This tutorial helps you understand what the library is doing and how to think about zkApp manager account.
...
The fungible token use case is just one example of possibilities. A different manager contract could, for example, store NFTs (or a commitment to NFTs) in the on-chain state of user accounts. In that case, the on-chain state would be the meaningful part of token accounts. The manager contract wouldn't care about constraining the balance change. Instead, the approval logic would contain checks that the on-chain state is only updated in a certain way:
...
The account updates for this transaction look like:

<img src="/img/tutorial-11-txn2.png" width="95%" />

This visualization shows both the account update for the fee payment and the main tree that does the token operations. You can see the call to `tokenUser` at the top, followed by a call to `myToken` that is doing the approving. 
...
Congratulations! You have explored some of the advanced features of account updates. You can now build with token zkApp manager accounts.
