...
The first line creates JavaScript code in the build directory, while the second line runs the code in `src/main.ts`. This command will fail at this point though, since we haven't written the `Square` smart contract yet.
