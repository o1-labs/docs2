...
This tutorial has been tested with Mina zkApp CLI version `0.6.4` and SnarkyJS `0.8.0`.

## Create a new project

Now that you have the tooling installed, you can start building your application.

First, create a new project using the `zk project` command:

```sh
zk project 01-hello-world
```

This command creates a directory named `01-hello-world` that contains the scaffolding for your project, including tools such as Prettier, ESLint, and Jest.

One of the features of the `zk project` command is the ability to scaffold the UI for your project. For this tutorial, select `none`:
...
Change into the `01-hello-world` directory and list the contents:
...
For this tutorial, you work mostly in the `src` directory that contains the TypeScript code for the smart contract. Each time you make updates, then build or deploy (which automatically builds), the TypeScript code is compiled into JavaScript in the `build` directory.
...
Then, generate the new files for the project:
...
In later tutorials, you learn how to interact with a smart contract from the browser, like a typical end user. For now, you just use the `main.ts` script.

Now, open `src/index.ts` in a text editor and change it to look like:
...
The first command creates JavaScript code in the build directory. The second command runs the code in `src/main.ts`.
...
This command runs `main` if the build is successful.
...
First, open `src/Square.ts` in your editor, then add the following code at the top of the file:
...
- `Field`: The native "number" type in SnarkyJS. You can think of these as unsigned integers. These fields are the most basic type in SnarkyJS and are what all other SnarkyJS-compatible types are built on top of.
...
zkApps can have up to 8 Fields of on-chain state, each storing up to 32 bytes (technically, 31.875 bytes or 255 bit) of arbitrary data. A later tutorial covers options for off-chain state.
