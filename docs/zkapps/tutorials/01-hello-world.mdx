...
The full source code for [01-hello-world](https://github.com/o1-labs/docs2/tree/main/examples/zkapps/01-hello-world) is provided in the `examples/zkapps` directory. 
...
Now try adding a transaction that should fail. Now that `num` is in state `9`, updating is possible only with `81`. To test a failure, update the state to 75 in `zkAppInstance.update(Field(75))`:
