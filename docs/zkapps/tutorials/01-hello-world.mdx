...
Open up `src/main.ts` in your editor. A complete version of this file can be found [here](https://github.com/es92/zkApp-examples/blob/main/01-hello-world/src/main.ts). Again, we recommend that you copy this whole file over to begin with, and then work through the tutorial steps with the complete file in place.
...
Now, let's initialize our smart contract. Comments are provided to break down each stage.
...
32     contract.sign(zkAppPrivateKey);
33   });
34   await deployTxn.send();
35
36   // Get the initial state of our zkApp account after deployment
37   const num0 = contract.num.get();
...
40   // ----------------------------------------------------
41
42   const txn1 = await Mina.transaction(deployerAccount, () => {
43     contract.update(Field(9));
44     contract.sign(zkAppPrivateKey);
45   });
46   await txn1.send();
47
48   const num1 = contract.num.get();
...
52
53   try {
54     const txn2 = await Mina.transaction(deployerAccount, () => {
55       contract.update(Field(75));
56       contract.sign(zkAppPrivateKey);
57     });
58     await txn2.send();
59   } catch (err: any) {
60     console.log(err.message);
61   }
62   const num2 = contract.num.get();
...
65   // ----------------------------------------------------
66
67   const txn3 = await Mina.transaction(deployerAccount, () => {
68     contract.update(Field(81));
69     contract.sign(zkAppPrivateKey);
70   });
71   await txn3.send();
72
73   const num3 = contract.num.get();
