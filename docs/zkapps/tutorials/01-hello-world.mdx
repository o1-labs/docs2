...
Using a simulated local blockchain speeds up development and tests the behavior of your smart contract locally. Later tutorials cover how to use a lightweight Mina network (Lightnet) in a single Docker container to test your zkApp before you deploy the zkApp to live Mina networks.
...
- The term _Lightnet_ is used to describe the lightweight Mina network (Lightnet) that is a more accurate representation of the Mina blockchain. See [Lightnet zkApp Testing](/zkapps/lightnet).
:::
...
This simulated local blockchain also provides pre-funded accounts. These lines create local test accounts with test MINA to use for this tutorial:
