...
The full source code for this tutorial is provided in  the [examples/zkapps/01-hello-world](https://github.com/o1-labs/docs2/tree/main/examples/zkapps/01-hello-world) directory on GitHub. While you're there, give the `/docs2` repository a star. 
...
This tutorial has been tested with Mina zkApp CLI version `0.7.5` and SnarkyJS `0.9.8`.
...
2. A local repository was created for you when you created a project using the `zk project` command, you can commit your changes. By saving your changes now, you can roll back changes if necessary:
...
For this tutorial, you run commands from the `01-hello-world` directory as you work in the `src` directory on files  that contain the TypeScript code for the smart contract. Each time you make updates, then build or deploy, the TypeScript code is compiled into JavaScript in the `build` directory.
...
The `zk file` command created the `src/Square.ts` and `src/Square.test.ts` test files. However, this tutorial does not include writing tests, so you just use the `main.ts` file as a script to interact with the smart contract and observe how it works. 
...
The following commands are included for reference only and will fail if you run them now. This is expected behavior because you haven't written the `Square` smart contract yet.
...
Because you haven't built the smart contract yet, this command fails as expected. 
...
After you create a working smart contract, this command runs `main` if the build is successful. When successful, the first command creates JavaScript code in the `build` directory. The second command runs the code in `src/main.ts`.

If you run the commands now, command failure is the expected result.  

## Write the zkApp Smart Contract

Now, the fun part! Write your smart contract in the `src/Square.ts` file. 

Line numbers are provided for convenience. A final version of the smart contract is provided in the [Square.tx](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/01-hello-world/src/Square.ts) example file.

This tutorial walks through the `Square` smart contract code in the `src/Square.ts` as viewed in a text editor. Because this is your first project, this tutorial uses the provided example file to ensure you enjoy the best user experience.  


## Copy the example 

This tutorial walks you through the [Square.tx](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/01-hello-world/src/Square.ts) example file. 

1. First, open the [Square.tx](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/01-hello-world/src/Square.ts) example file.
...
Now, review these lines to learn (... what?...). 
...
- `Field`: The native number type in SnarkyJS. You can think of Field types as unsigned integers. Fields are the most basic type in SnarkyJS. All other SnarkyJS-compatible types are built on top of Field types.
...
This method contains the logic by which end users are allowed to update the zkApp's account state on chain. In this example, the code specifies:
...
Next, write a script that interacts with your smart contract. As you did earlier, copy the `main.ts` example file and then work through the tutorial steps with the complete file in place. 

1. First, open the [main.ts](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/01-hello-world/src/main.ts) example file.

1. Copy the entire contents of the file into your smart contract in the `src/main.ts` file.

Now you are ready to review the script that interacts with your smart contract.

### Imports

The `import` (?what?) does (?what?).

Review these lines to learn (...what?):
...
The following code in the `src/main.ts` adds (? what?):
...
Now that the Square smart contract is complete, you can successfully run these commands from the root directory of your project:
...
In production, zkApps are deployed to Mina network. To learn more, see [How to Deploy a zkApp]](https://docs.minaprotocol.com/zkapps/how-to-deploy-a-zkapp). 

Using a "local blockchain" speeds up development and tests the behavior of your smart contract locally. This local blockchain also provides pre-funded accounts (see the `deployerAccount` code). Later tutorials cover how to deploy a zkApp to live Mina networks.

### Initializing your smart contract

Now, review the code in the `main.ts` file that initializes your smart contract. Comments are provided to break down each stage.
...
All smart contracts that you create with the zkApp CLI uses similar code.
...
Now you see how to update your local zkApp account with a transaction. 

Add the following code to the `src/main.ts` file:
...
Now try adding a transaction that fails. 
The contract logic allows the number that is stored as on-chain state to be replaced only by its square. Now that `num` is in state `9`, updating is possible only with `81`. To test a failure, update the state to 75 in `zkAppInstance.update(Field(75))`:
...
And finally, to show the correct update: (?what is changed here?)
...
Congratulations! You have successfully walked through all of the steps to build your first zkApp with SnarkyJS.
