---
title: "Tutorial 4 [Draft]: Building a zkApp in the Browser with React"
hide_title: true
sidebar_label: "Tutorial 4 [Draft]: Building a zkApp in the Browser with React"
---

# Tutorial 4 [Draft]: Building a zkApp in the Browser with React

:::caution

This tutorial is currently a draft. It is currently public for testing, but only minimal support for its contents will be provided as it is likely to change soon, as new features are added to SnarkyJS and the tutorial undergoes further review.
...
In this tutorial, we will look at a react app that interacts with zkApps from the browser. There are two ways to use this tutorial and its sample code. 

First, you can use this project as an example of how to build a React app, that works with zkApps, and runs in the browser.

Second, this example itself can be a convenient starting point for your own zkApp. It is split into two components, `Initialize`, and `SmartContractUI`. If you would like to build a browser UI for your own zkApp, you can reuse much of the project, and primarily edit `SmartContractUI`.
...
Unlike previous tutorials, we will not be going over all of the code for this example. You can find the full source code [here](https://github.com/es92/zkApp-examples/tree/main/04-zkapp-browser-ui). Most of the code is similar to the previous tutorial, [Tutorial 3](deploying-to-a-network), or follows usual react conventions. Given that, we will be focusing this tutorial on key, unique parts of the code.
...
* This tutorial runs SnarkyJS on the UI thread, which blocks and makes the page unresponsive while doing complex computations like proving zero knowledge proof transactions. A future version is planned to use Web Workers to fix this, as well as other optimizations to in browser proving.
...
* A new version of the zkApp CLI is in the works which will let you develop your Smart Contract and UI in a monorepo. The version discussed here has the CLI deployed separately.
...
This will automatically run whenever you run the local react server, enabling SharedArrayBuffers for our application, and in doing so allow SnarkyJS to run.

### Smart Contract

You can find the code for the smart contract we will be using [here](https://github.com/es92/zkApp-examples/tree/main/04-zkapp-browser-ui-smart-contract). This smart contract just stores a number, and calling its function adds one to the number.

To connect this smart contract, we will be making one change from normal. In `src/index.ts`, we have added a line so the file looks like:
...
Inside the `04-zkapp-browser-ui-smart-contract` folder, run:
...
```sh
npm link 04-zkapp-browser-ui-smart-contract --save
```
...
Whenever you save one of the files in `src`, the react project will refresh.
...
106
107     let { verificationKey } = mockEffects ? { verificationKey: null } : await Add.compile();
108
109     let zkapp = new Add(zkAppAddress);
110
...
This component has a pane for interacting with the smart contract, and a pane for showing the progress of producing and sending transactions. If you want to make your own application, besides changing the smart contract, you would modify the code and add your UI in this component.
...
* add a homepage variable to package.json (`"homepage": "./"`). See the example's package.json for more.
