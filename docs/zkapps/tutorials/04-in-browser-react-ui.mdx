...
:::caution
This tutorial currently uses localStorage to store and manage the user's private key. While helpful for prototyping applications, we do not recommend using it in production, since it does not provide only limited guarantees around persistence and security. 

A new version of SnarkyJS is in the works which will connect to Auro wallet for key management in production applications.
:::
