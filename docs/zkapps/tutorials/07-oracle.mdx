...
In this case, the event is `Field`, but it could be a more complicated type built on Fields, if the situation called for it. Emitted events are stored and available on [archive nodes](/node-operators/archive-nodes/archive-node) in the Mina network.
