...
<<<<<<< HEAD
In this case, the event is `Field`, but it could be a more complicated type built on Fields, if the situation called for it. Emitted events are stored and available on [archive nodes](/node-operators/archive-node/getting-started) in the Mina network.
=======
...
You can find the complete code for this example [here](https://github.com/o1-labs/docs2/tree/main/examples/zkapps/07-oracles).
