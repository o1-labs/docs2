...
You can view the complete oracle code [here](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/07-oracles/oracle/app/api/credit-score/route.ts) and can generate a Mina-compatible public/private key pair for your oracle by running `npm run keygen` (which will run the code in [this file](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/07-oracles/oracle/scripts/keygen.js)).

For our oracle, we’re using a Vercel Serverless Function. It’s very simple, but you don’t have to dive into the code now. It’s commented to explain each step so you can build something similar for yourself!
...
For purposes of this tutorial below, you can access our demo oracle at https://07-oracles.vercel.app/api/credit-score?user=1 and https://07-oracles.vercel.app/api/credit-score?user=2, if you don’t want to set up your own oracle right now.
