...
This oracle uses a Koa server hosted on DigitalOcean that a computer can interact with. It's short, but you don't have to dive into the code now. The `index.js` code is commented to explain each step so you can use it as a model to build something similar.
...
The demo oracle for user 1 is available at https://mina-credit-score-signer-pe3eh.ondigitalocean.app/user/1 shows this response:
...
The demo oracle for user 2 with a lower credit score is available at https://mina-credit-score-signer-pe3eh.ondigitalocean.app/user/2 shows this response:
...
You can modify the example code in `index.js` to create your own Koa oracle on DigitalOcean to deploy in one click, or use these demo oracles for educational purposes.
...
This completes the basic setup for the smart contract. For details on the `deploy()` method, see [Tutorial 1: Hello World](hello-world).
...
After the smart contract is deployed, run the `init()` method before users have the chance to interact with it to set the initial on-chain state and other configuration. You can think of it like a constructor in a Solidity contract.

### Emit Events

The smart contract checks that a user has a credit score above a certain threshold. But, how do can the user prove it? 
...
You want the `verify()` method to emit an event only if the user's credit score is 700 or higher. To ensure that this condition is met, call `assertGte()` (assert greater than or equal to) on `creditScore`.
...
You might have to try it twice in order for all the tests to pass because you are calling the live API directly. 
