...
A CLI tool is provided for convenience for deploying smart contracts to networks. If you would like to see how to do this programmatically, see the file [here](https://github.com/es92/zkApp-examples/blob/main/03-deploying-to-a-live-network/src/deploy.ts). This is useful if you need more custom account creation - say deploying a zkApp to a different key than the fee payer key, programmatically parameterizing a zkApp before initializing it, or creating a Smart Contract programmatically for users as part of an application.
...
We will be using a set of helper functions to make this more convenient to write, in a `utils.ts` file. You can find this file [here](https://github.com/es92/zkApp-examples/blob/main/03-deploying-to-a-live-network/src/utils.ts). Please download this and place it in your project's `src` folder. It contains 3 functions:
