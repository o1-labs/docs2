...
When a user interacts with this smart contract by providing a proof, the proof is generated locally on the user's device, and included in a transaction. Mina recursively checks the proof, to know it is correct and matches the verification key, and to bundle that proof into Mina's recursive ZKP. 
...
...
 ...
 24
 25   const configName = process.argv[2];
 26
 27   const deployerKeysFileContents = fs.readFileSync(
 28     'keys/' + configName + '.json',
 29     'utf8'
 30   );
 31
...
This uses the key generated by the `zk config` tool, stored in `keys/`. The name of the key file will be provided through an argument on the command line (`process.argv[2]`). 
...
Our Smart Contract is also stored on chain in the same account we deployed from. So we set `zkAppPrivateKey = deployerPrivateKey`.
...
```
 ...
 We wait until the deployer's account exist - if it does not, we share a link with the user to go to the faucet and request funds.

If you've already run `zk deploy` as described earlier in this tutorial, then this should find the account instantly and move on to the next step. But its good practice to have as a check, and useful if you're writing code for other flows, say a CLI application that interacts with an existing zkapp, where the user's key may not have funds yet.
...
...
 ...
 70   //   more custom account creation - say deploying a zkApp to a different key than the fee payer
