...
Now you can see how to put these ideas into practice as you walk through the
...
For example, a use case for this semi-anonymous contract is to enable DAO member to make credible statements on behalf of their DAO without revealing their specific individual identity.
...
1. Create the project:
  ...
  This command creates a directory with a new project template that is fully set up
  and ready for local development.

### Prepare the project

Start by deleting the example smart contract files that come with the new project.
...
1. Create the `src/message.ts` file and generate the corresponding `src/message.test.ts` test file:

  ```sh
  $ zk file message
  ```

1. Open the `message.ts` file and paste in the following:
...
The `@method` decorator tells o1js that to allow users to call this method and to generate a zero-knowledge proof of its execution.
