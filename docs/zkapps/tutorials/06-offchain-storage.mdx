...
Note that it contains logic for both running the contract locally and for deploying and interacting with it on Berkeley. This is a useful pattern when developing a new contract. If you would like to try it on Berkeley, deploy our contract as usual with `zk deploy`, set `useLocal` to false in `main.ts`, and add the name of your config to your call to the end of your call to `node main.js`.
...
144     } else {
145       priorLeafNumber = Field(0);
146       newLeafNumber = Field(1);
147     }
...
```
...
183       );
184
185       await updateTransaction.send();
...
```
