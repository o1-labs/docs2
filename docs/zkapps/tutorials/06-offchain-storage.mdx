...
Why do we need this? When building an application we'll just use locally and for testing, its fine to just build and store a merkle root locally. 
...
* Single-server offchain storage [this solution].
...
We're looking for a developer to run with the library presented here, improve it, and run instances for the community. There is a grant opportunity for taking this on. If interested, please reach out [here](where_should_they_reach_out?).

Some suggested improvements:

* Eliminate DDOS vulnerability by adding a token that limits storage requests
* Add support for trees of variable heights (currently only supports trees of height 8, or 128 leafs)
* Surface errors to the client library
...
* Write a more scalable database (eg Redis)
* Writing a storage server for an automatically scalable service (eg Cloudflare)

## Implemeting a Project Using Offchain Storage
