...
In this example, we are spinning up the Mina chain locally and deploying all three smart contracts locally.

Then we are calling `callAddAndEmit` method from `Caller` smart contract which will take two numbers as arguments, then call the `Adder` smart contract which will add these two numbers and pass the result in `Incrementer` smart contract which will increment the result by 1.

In our case we should see state equal to 12.
