---
title: 'Example: Oracle'
hide_title: true
sidebar_label: 'Example: Oracle'
---
...
You can view the complete oracle code [here](https://github.com/jackryanservia/credit-score-signer-vercel/blob/main/pages/api/credit-score/%5Bid%5D.ts) and can generate a Mina-compatible public/private key pair for your oracle using [this script](https://github.com/jackryanservia/credit-score-signer-vercel/blob/main/pages/api/keygen.ts).

For our oracle, we’re using a serverless function hosted on Vercel. It’s short, but you don’t have to dive into the code now. It’s commented to explain each step so you can build something similar for yourself!
...
If you decide to deploy to a serverless environment like we did here, you will need to keep the service’s memory and runtime limitations in mind. Our credit score example uses about 500 MB of memory and takes nearly 30 seconds to cold start (though subsequent queries are much faster). Many services (including the free version of Vercel Functions) do not provide enough memory to run SnarkyJS. For our demo, we are using a paid Vercel plan.
...
```json
{
  ...
  "signature": {
    "r": "2366821166503338420677946724260579195978574100722996189867656398635505142016",
    "s": "11923525266274708837554293860277013650841825394090771233039348901942148007140"
  },
  "publicKey": "B62qp24xA8az6sZbCFLaswALCKZLYWSErw4rAbej3b4aoLqumamcSAu"
}
```
...
```json
{
  ...
  "signature": {
    "r": "18843685804321949248196684491815874666837415778094547309423509340222290135938",
    "s": "7043007656800553195096070050281121019779790649880562873973377281060733231107"
  },
  "publicKey": "B62qp24xA8az6sZbCFLaswALCKZLYWSErw4rAbej3b4aoLqumamcSAu"
}
```
...
For purposes of this tutorial below, you can access our demo oracle at https://credit-score-signer-vercel.vercel.app/api/credit-score/1 and https://credit-score-signer-vercel.vercel.app/api/credit-score/2, if you don’t want to set up your own oracle right now.
...
Open up `/src/CreditScoreVerifier.ts` and paste in the following:
  ...
  describe('actual API requests', () => {
    it('emits an `id` event containing the users id if their credit score is above 700 and the provided signature is valid', async () => {
      ...
      const response = await fetch(
        'https://credit-score-signer-vercel.vercel.app/api/credit-score/1'
      );
    ...
    it('throws an error if the credit score is below 700 even if the provided signature is valid', async () => {
      ...
      const response = await fetch(
        'https://credit-score-signer-vercel.vercel.app/api/credit-score/2'
      );
      const data = await response.json();
  ...
  describe('hardcoded values', () => {
    it('emits an `id` event containing the users id if their credit score is above 700 and the provided signature is valid', async () => {
      ...
      const signature = Signature.fromJSON({
        r: '16545513748775911233424851469484096799413741017006352456100547880447752952428',
    ...
    it('throws an error if the credit score is below 700 even if the provided signature is valid', async () => {
      ...
      const signature = Signature.fromJSON({
        r: '27495843668796808617555742421380880633422629203586406510462736184339334351220',
        s: '11193631805202176804739844986053762387244537637721683730748300387148132134317',
      });
...
Congratulations! You have just built a simple oracle using SnarkyJS and the Mina Protocol.
