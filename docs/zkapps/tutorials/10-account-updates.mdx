...
Each zkApp transaction constructed by o1js is composed of one or more [AccountUpdates](../o1js-reference/classes/AccountUpdate), which are a set of instructions for the Mina network to perform, such as altering on-chain state, emitting an event, etc.
