...
Each zkApp transaction constructed by o1js is composed of one or more [AccountUpdates](../o1js-reference/classes/AccountUpdate) classes, which are a set of instructions for the Mina network to perform, such as altering on-chain state, emitting an event, and so on.
...
Permissions, preconditions, composability, and tokens are the core features of zkApps and are implemented using `AccountUpdates`. 

In this tutorial, you learn the essential AccountUpdates features.

To learn more, see these o1js docs:

- [Permissions](/zkapps/o1js/permissions)
- [On-Chain Values](/zkapps/o1js/on-chain-values) <!-- why is it called precondition? the link text did not match the target page -->

## AccountUpdate contents

An `AccountUpdate` is a set of instructions for the Mina network. It includes preconditions <!-- why is it called precondition? --> and a list of state updates that need to be authorized by a signature or proof.
...
- `Authorization`: How the zkApp is authorized; must be a proof (corresponding to the verification key on the account) or a signature.
...
If the verification key cannot be changed, the zkApp smart contract is non-upgradeable. The `setVerificationKey` permission sets the ability to change the verification key of the account.
...
The full source code for this tutorial is provided in [Account Updates](https://github.com/o1-labs/docs2/tree/main/examples/zkapps/10-account-updates).

## Smart Contracts

In this tutorial, you build two smart contracts: 

- `ProofsOnlyZkApp.ts`: Non-upgradeable proof only
- `SecondaryZkApp.ts`:  <!-- do we want to call these anything else? keep the .ts?>

### Non-upgradeable proof only

The full example code is provided in the [ProofsOnlyZkApp.ts](https://github.com/o1-labs/docs2/tree/main/examples/zkapps/10-account-updates/src/ProofsOnlyZkApp.ts) file.
