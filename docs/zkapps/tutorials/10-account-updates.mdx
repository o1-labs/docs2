...
Each zkApp transaction constructed in SnarkyJS is composed of "AccountUpdates".
...
Many of the core features of zkApps - permissions, preconditions, composability, and tokens - are each implemented through use of AccountUpdates. In this tutorial, we'll discuss all of these except tokens and assertions on account update layouts, which will be discussed in a later tutorial.

You can find more info on these particular items elsewhere in the docs:
...
Each [AccountUpdate](../snarkyjs-reference/classes/AccountUpdate) has several components, the following we'll be discussing here:

* `PublicKey`: The account address for the account update
* `TokenId`: By default the Mina TokenId. Between this and PublicKey, uniquely determines the account
...
* `MayUseToken`: Whether or not the zkApp has permissions to manipulate its token.
* `Layout`: Allows for assertions about the structure of an AccountUpdate.

## AccountUpdates for a non-user-upgradable zkApp

We'll now be building an example zkApp, to explore permissions, preconditions, and composability with AccountUpdates.
...
You can find all of the code for this tutorial [here on github](https://github.com/es92/zkApp-examples/tree/main/10-account-updates).

### Smart Contracts

We'll be buliding two smart contracts to use in our example:

First, [ProofsOnlyZkApp.ts](https://github.com/es92/zkApp-examples/blob/main/10-account-updates/src/ProofsOnlyZkApp.ts).

This is a zkApp configured to only be modifiable or modifiable via proofs (and for the zkApp we're deploying, is not upgradable once deployed). This means that while someone must own the private key to deploy the zkApp itself - after deployed, only the rules in the zkApp determine how the zkApp functions. The private key is no longer useful for anything.

This zkApp also has functions that call other functions, so we can explore how that impacts a transcation's AccountUpdates.

We'll start by adding the main contents of the zkApp:
...
This configures the zkApp as described, and initializes the zkApp with the values we'd like. 

:::caution
Note we assert that `provedState` is `False` in `init`. This ensures that `init` cannot be called again after the zkApp is setup. Without this, our zkApp could be reset by anyone calling the `init` transaction on our zkApp, and is recommended for most zkApps.
:::

Next, we'll add two functions:
...
:::caution
In each of these, we are also asserting `provedState` is `true`. This ensures the zkApp was initialized as expected, as `provedState` becomes true after `init` is called, and is recommended for most zkApps.
:::

Note we are also having the `add` method call the `incrementCalls` method, to see how this is reflected in an `add` transaction's AccountUpdate structure.

Lastly, we'll add one more function, `callSecondary`, which calls a different zkApp:
...
This takes the address of our other zkApp, `SecondaryZkApp`, and calls a method on it. Note that the impact of calling that method will occur after this set of AccountUpdates - so when we call `secondaryContract.num.get()`, that will get the value before this transaction was applied.

Last, to look briefly at [SecondaryZkApp.ts](https://github.com/es92/zkApp-examples/blob/main/10-account-updates/src/SecondaryZkApp.ts), we have contents:
...
We'll now discuss the file [main.ts](https://github.com/es92/zkApp-examples/blob/main/10-account-updates/src/main.ts), which creates transactions with the above smart contracts, and the account update visualizations it creates.

First, we'll import the transaction visualizer:
