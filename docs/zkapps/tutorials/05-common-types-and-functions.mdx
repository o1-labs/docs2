...
Merkle trees in smart contracts are stored as the hash of the Merkle tree's root. Smart contract methods that update the Merkle root can take a _witness_ of the change as an argument. The witness represents the Merkle path to the data for which inclusion is being proved.
...
See the API reference documentation for [Merkle maps](../snarkyjs-reference/classes/MerkleMap). These allow us to implement key value stores.
