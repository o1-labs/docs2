...
Lastly, let's go over an example using [merkle maps](../snarkyjs-reference/modules/Experimental#merklemap). These allow us to implement key value stores.
...
...
  ...
  @method update(
    keyWitness: Experimental.MerkleMapWitness,
    keyToChange: Field,
    valueBefore: Field,
    incrementAmount: Field,
  ) {
