...
A Merkle tree is a binary tree where each node is the hash of its children, and the root node represents the summary of all other nodes in the tree. Merkle trees are especially useful because they can be used to validate large sets of data efficiently.
...
To use more data from your zkApp, check out [Tutorial 6: Off-Chain Storage](/zkapps/tutorials/offchain-storage) to learn how to use off-chain storage.
