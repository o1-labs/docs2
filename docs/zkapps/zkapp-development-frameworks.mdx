...
- zkApps will be subject to protocol throughput limitations.
...
|**Execution environment**|Execution off-chain, verification on-chain, transaction ordering possible on-chain.|Hybrid execution model, both on-chain (sequencer) and off-chain thanks to recursive zk-proofs, verification on-chain (MINA L1).|
