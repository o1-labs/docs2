...
To view all zkApp CLI commands, install it with `npm install -g zkapp-cli` and then run `zk --help`.
...
- [Events](/zkapps/events) — writing events and [reading them from an archive node](/zkapps/advanced-snarkyjs/fetch-events-and-actions)
- [Actions & reducer](/zkapps/snarkyjs/actions-and-reducer)
- [Encryption and decryption](/zkapps/snarkyjs-reference/modules/Encryption) — using Mina's curves
- [Merkle tree](/zkapps/snarkyjs/merkle-tree)
- [Custom tokens](/zkapps/snarkyjs/custom-tokens)
- [Using on-chain values](/zkapps/snarkyjs/on-chain-values) — a.k.a. protocol and account preconditions
