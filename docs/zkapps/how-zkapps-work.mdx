...
zkApps are written in TypeScript using the Mina zkApp CLI. In the [how to write a zkApp](how-to-write-a-zkapp) section, you learn about writing your first zkApp. First, learn  how zkApps work.
...
zkApp developers usually build a UI to allow users to interact with the smart contract.

Typically, this static website is deployed to a host of the developer’s choosing.  Recommend web hosts offer a global CDN to ensure the best user experience.

Your website must contain the `smart_contract.js` file that was generated when building your smart contract. See [how to write a zkApp](how-to-write-a-zkapp) page for more detail.
...
When the prover function runs in a web browser, the smart contract outputs a proof and some associated data called “events”. This is sent as part of the transaction when sending a transaction to a zkApp address. These events are a JSON plaintext description that describes how to update the state on a zkApp account.

The integrity of these events is ensured by passing a hash of these events as a public input to the smart contract. The events must be present and unmodified for the verification function to pass successfully when it runs on Mina. In this way, the Mina network can confirm the integrity of both the proof and the associated events which describe how to update the zkApp account’s state.
