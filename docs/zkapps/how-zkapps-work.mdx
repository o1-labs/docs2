...
zkApps are written in TypeScript using the Mina zkApp CLI. In the upcoming [how to write a zkApp](how-to-write-a-zkapp) section, you learn the details about writing your first zkApp. For now, get started by learning how zkApps work.
...
zkApp developers usually build a UI to allow users to interact with the smart contract and then deploy this interactive UI as a static website. To achieve the best user experience, zkApp developers deploy the UI static website to a recommended web host that offers a global content delivery network (CDN).

Your website must contain the `smart_contract.js` file that was generated when you built the smart contract. For more detail, see [how to write a zkApp](how-to-write-a-zkapp).
