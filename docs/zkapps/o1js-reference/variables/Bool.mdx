```ts
const Bool: typeof Bool & (...args) => Bool;
```

A boolean value. You can create it like this:

## Example

```
const b = Bool(true);
```

You can also combine multiple Bools with boolean operations:

## Example

```ts
const c = Bool(false);

const d = b.or(c).and(false).not();

d.assertTrue();
```

Bools are often created by methods on other types such as `Field.equals()`:

```ts
const b: Bool = Field(5).equals(6);
```

## Source

[lib/provable/wrapped.ts:70](https://github.com/o1-labs/o1js/blob/ccbd5fd4525adaf6c1f07bef7847d7bac67c71b5/src/lib/provable/wrapped.ts#L70)
