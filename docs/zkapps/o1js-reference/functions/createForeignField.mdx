...
Create a class representing a prime order finite field, which is different from the native [Field](../type-aliases/Field.mdx).
...
- create your field elements using the [ForeignField.AlmostReduced](../classes/ForeignField.mdx#almostreduced) constructor, or using the `.provable` type on that class.
```ts
...
To convert to a canonical field element, use [ForeignField.assertCanonical](../classes/ForeignField.mdx#assertcanonical):
...
[lib/provable/foreign-field.ts:627](https://github.com/o1-labs/o1js/blob/ccbd5fd4525adaf6c1f07bef7847d7bac67c71b5/src/lib/provable/foreign-field.ts#L627)
