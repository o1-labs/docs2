...
One difficult issue with learning OCaml is locating and reading documentation for libraries. One library in particular is the `Core` library that has the following structure:
...
There are actually multiple sources for `Core` documentation. To find the correct documentation, first go the GitHub then use the codebase to locate the correct documentation. If you can't find the module you're looking for go to `Core_kernel` and if that fails then look for `Base`. The find capability does work if the sections are not expanded.

Instead of using the HTML documentation, a better way is to use editor integration using a tool like `merlin` that provides type hints. One downside though is that Merlin works only if your code compiles. It also gives us code navigation like go to definition.

OPAM, the package manager for OCaml, usually ships documentation with libraries. This can be accessed through `merlin`.
...
In the above example, the single `@` scopes an extension to a single expression, and the `@@` denotes the extension is expanded in the scope of the calling context.
...
Under the hood, Async uses Monads however Ivar's are the primitive. `a' Ivar.t` is essentially a mutex that can only be filled once. After the value from a computation returns, it then fills the Ivar. The rest of the syntactic sugar takes the Ivar and passes them through `Deferred` monads.
