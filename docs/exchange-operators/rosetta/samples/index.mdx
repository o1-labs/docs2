...
Also for using `pprint_json` function, you will need `jq` installed on your machine. On Linux machines, it is installed by default. For macOS, you can install it with brew package manager:
...
Let's start with necessary imports and define constants:
...
As we are going to work with json requests and responses, we may want to print highlighted and indented json objects. Let's define a simple function doing that with help of `jq` utility
