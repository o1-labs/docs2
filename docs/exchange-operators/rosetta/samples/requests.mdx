...
In this article you can learn how Mina-specific Rosetta json objects look like
We will also implement some helper functions to construct and parse such objects.
...
Let's implement generic `request` function to perform a Rosetta API calls injecting that object under the hood:
...
Let's implement helpers to construct such objects:
...
<details><summary>See sample json object representing an operation</summary>
...
In Mina, each operation represents an [Account Update](../../../zkapps/o1js/interact-with-mina#account-updates). Therefore, a MINA token transfer transaction would be represented by three Account Updates:
- for decreasing fee payer account balance (the fee payers' account would be the same as senders')
- for decreasing sender account balance
- for increasing receiver account balance

<details><summary>See sample object representing a transfer transaction</summary>
...
According to the Rosetta specification, the array of `operation` objects should be passed as a parameter in the `/construction/preprocess` and `/construction/payloads` endpoints.

Let's implement a helper function to construct the operations array for a MINA token transfer:
...
Now when we have all helpers in place, lets implement all Rosetta client methods according to the specification:
