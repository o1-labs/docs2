...
For a quick test, lets run `/network/list` request:
...
According to the Rosetta specification, one should use offline tool to sign transactions, and use Rosetta to submit the signed ones.
Mina Rosetta implementation comes with an offline signer tool. One can generate a keypair, sign a transaction and verify a signature with it.
...
To simplify examples, we will refer to it by `signer` alias.
...
To sign a transaction, simply run:

    signer sign --private-key <private key> --unsigned-transaction <tx>

:::note 

`<tx>` should be replaced by unsigned transaction string, which you can construct using `construction/payloads` endpoint.
