...
3. For each block produced that has a non-zero block-reward on the canonical chain rewards should be calculated based on 720 MINA. 
...
1. Account A retains: 0.2 * 720 MINA * 3 blocks = 432 MINA
2. Mina Foundation, Account B, payout should be: (0.6 * 0.92) * 720 MINA * 3 blocks = 1,192.32 MINA. 
3. Account A retains 8% or (0.6 * 0.08) * 720 MINA * 3 blocks = 103.68 MINA 
3. Account C payout should be: (0.2 * 0.92) * 720 MINA * 3 blocks = 397.44 MINA. 
4. Account A retains 8% or (0.2 * 0.08) * 720 MINA * 3 blocks = 34.56 MINA
