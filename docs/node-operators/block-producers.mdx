...
To further incentivize staking, if the account that wins the slot is one that does not have any [time-locked tokens](./time-locked-accounts), the block producer will receive double the coinbase rewards.
