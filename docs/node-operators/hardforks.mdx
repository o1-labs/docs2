...
- A hard fork means the old chain is incompatible with the new chain. After a hard fork it is possible for two chains to exist. To avoid splitting the chain it is important that every node on the network updates to the latest software. For example, Bitcoin Cash is considered to be the most successful hard fork of the primary cryptocurrency.
