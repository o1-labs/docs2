...
4. [Archive Nodes](https://docs.minaprotocol.com/node-operators/archive-node) - A regular mina daemon that is connected to a running archive process. The daemon regularly sends blockchain data to the archive process that stores it in a [PostgreSQL](https://www.postgresql.org/) database.
