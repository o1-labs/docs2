...
If you are interested in receiving a grant to develop Mina for Windows, send an email to grants@o1labs.org or reach out in the [Mina Protocol Discord server](https://discord.gg/minaprotocol).
...
If the `-external-ip YOUR_IP` flag is not provided, the daemon uses HTTPS (443) and HTTP (80) to determine its own IP address.
