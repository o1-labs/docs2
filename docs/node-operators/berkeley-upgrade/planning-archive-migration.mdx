...
The archive database must be migrated in two steps. Each step uses an application that is shipped by Mina Foundation and O(1) Labs teams:
...
2. **replayer application in migration mode**
...
To automatically verify and patch missing blocks, use the [download_missing_blocks.sh](https://raw.githubusercontent.com/MinaProtocol/mina/2.0.0berkeley_rc1/src/app/rosetta/download-missing-blocks.sh) script. Because the `download-missing-blocks` script uses localhost as the database host, you must run it from within the database host.
...
- [How to perform archive migration](/node-operators/berkeley-upgrade/migrating-archive-database-to-berkeley)
