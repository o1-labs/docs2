...
The Mina archive node offers the ability for node operators to store data using any data store of their choice. However, archive nodes do not expose a built-in API to query this data publicly. Archive node operators can run the [Mina Archive Node API](https://github.com/o1-labs/archive-node-api) to provide a GraphQL API from which zkApp smart contracts can retrieve events and actions.

When using SnarkyJS, developers specify a Mina Archive Node API endpoint to integrate with their zkApp, allowing seamless access to archived events and actions.
