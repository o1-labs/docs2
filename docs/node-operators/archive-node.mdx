...
Since Mina nodes do not store historical network information and events/actions are not kept on-chain, emitted events/actions are preserved only in the archive node. Consequently, a zkApp can retrieve previously emitted events and actions from one or more Mina archive nodes.
...
When using SnarkyJS, developers specify a Mina Archive Node API endpoint to integrate with their zkApp, allowing access to archived events and actions.
