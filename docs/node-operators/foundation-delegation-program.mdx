...
3. For each block produced that has a non-zero block-reward on the canonical chain, calculate the payout by multiplying the non-supercharged coinbase reward (equal to ```720 MINA``` at Mainnet launch) by the provider share calculated in the previous step minus a 8% percent fee. (i.e. ```payout = (provider_share * 0.92) * coinbase)```
