...
Reminder that this section is intended for node operators. If you want to store, send, and receive MINA without running a node, please see the [Install a Wallet](/using-mina/install-wallet) page for links to various user-friendly wallets available for Mina.
...
To generate a signed transaction offline if your private key is on a Ledger device, see [Ledger Offline Mode](./ledger-app-mina#offline-mode").
...
We are assuming in the rest of section that you have the Mina client installed on your system, if you do not have Mina installed please see the [Getting Started](./getting-started).
...
Run the following command to import your [previously generated](./keypair) keypair file:
...
Once you feel comfortable with the basics of creating an address, and sending & receiving mina, we can move on to the truly unique parts of the Mina network like [participating in consensus and helping compress the blockchain](./staking).
...
All information here is relevant as of the 1.3.0 build:
...
If you believe you were temporarily disconnected from the network, but your node stayed online (i.e. the gossip network may have missed one or more of your transactions), as of the 1.3.0 build, you can resend any of the transactions locally and they will be broadcasted again to the network even if your node thinks they've already been shared.
