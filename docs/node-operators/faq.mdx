...
You can download all the software required to run a Mina node [here](node-operators/getting-started#installation). Right now, we have builds that work on macOS, as well as Debian 9 and Ubuntu 18.04. You can also [build from source](https://github.com/minaprotocol/mina) or run via [Docker](node-operators/connecting#docker) for other unsupported systems.
...
Head over to the [Docker](node-operators/connecting#docker) [Devnet page](node-operators/connecting-devnet) to learn more and get started.
...
See the [documentation for zkapps](architecture/zkapps), Mina's Snarkified applications.
...
[See here](node-operators/connecting#ubuntu-1804--debian-9-1) for
...
![Visualization of the Mina Blockchain and its Singleton Forks](/static/img/faq/mina-singleton-forks.png)
