...
Node operators participate in consensus to create new blocks and help compress data by generating zk-SNARKs.
...
See [Logging](./logging) to learn where the log file are located and how to export log files.  
...
Run the `mina client status` command. This will output the MIna daemon status. 

If your block producer is running, a block of text is returned which will contain a line similar to this:
```text
...
When starting up `mina daemon`, use the `-snark-worker-parallelism` flag. This is Equivalent to setting OMP_NUM_THREADS, but doesn't affect block production.
