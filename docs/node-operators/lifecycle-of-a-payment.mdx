...
hide_title: true
description: In Mina, payments pass through several steps before they are  verified and complete.
keywords:
  - payments
  - gossip protocol
  - transition block
...
- If the transaction pool hits its capacity, or `max_txpool_size`, then it evicts the transaction with the lowest fee in the pool, causing it dumped from memory. If this happens, the sender needs to resend the transaction with a higher fee, according to market dynamics at the time.
