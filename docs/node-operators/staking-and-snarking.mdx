...
Use the `mina client delegate-stake` subcommand to set or change the block producer key. See the [Mina Client](/node-operators/mina-cli-reference#mina-client) CLI help.
