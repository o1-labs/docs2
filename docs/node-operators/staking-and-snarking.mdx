...
To stake with your MINA, use the `delegate-stake` subcommand:
...
- `-receiver <DELEGATE-PUBLIC-KEY>` is the public key of the validators to receive your stake delegation
- `-sender $MINA_PUBLIC_KEY` is is the public key of the account from which you want to delegate
- `-fee 0.1` is the transaction fee required to record the transaction
