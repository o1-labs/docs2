...
In addition to [installing the Mina daemon on your machine](/node-operators/block-producer-node/getting-started), you may also want to have some system set up to archive historical information on your machine. To do so you can setup an [archive node](/node-operators/archive-nodes/archive-node), archive by using [other redundant means](/node-operators/archive-nodes/archive-redundancy), or both.
