...
Set the transaction level after you create the [database](/node-operators/archive-nodes/getting-started) and before you connect an archive process to it.
...
When you have block data (precomputed or extensional) available from [Back up block data](/node-operators/archive-nodes/archive-redundancy#back-up-block-data), you can restore missing blocks in an archive database using the tool `mina-archive-blocks`. 
...
1. Restore precomputed blocks from: 
  ...
  - [Save block data from logs](/node-operators/archive-nodes/archive-redundancy#save-block-data-from-logs)
