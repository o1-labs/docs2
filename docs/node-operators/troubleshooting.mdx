...
If you use `systemd` to manage the Mina daemon then it will not automatically restart on a reboot of the machine. You can manually run the service with `start` via `systemctl --user start mina` or start the service automatically at boot with `enable` via `systemctl --user enable mina`.
...
Transactions can now be cancelled see [Mina Client](./mina-cli-reference#mina-client) or wallet instructions.
