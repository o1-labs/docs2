...
To generate a signed transaction offline if your private key is on a Ledger device, see [Ledger Offline Mode](./ledger-hardware-wallet#offline-mode).
...
Run the following command to import your [previously generated](generating-a-keypair) keypair file:
...
Once you feel comfortable with the basics of creating an address, and sending & receiving mina, we can move on to the truly unique parts of the Mina network like [participating in consensus and helping compress the blockchain](staking-and-snarking).
